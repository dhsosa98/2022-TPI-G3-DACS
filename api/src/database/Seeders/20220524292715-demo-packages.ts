'use strict';

module.exports = {
  async up(queryInterface, Sequelize) {
    return queryInterface.bulkInsert('Packages', [
      {
        name: 'HELIUM',
        quantPeople: 2,
        ticketId: 73,
        hotelId: 54,
        insuranceId: 1,
        showId: 13,
        total: 55475.67,
      },
      {
        name: 'populus deltoides pollen',
        quantPeople: 6,
        ticketId: 94,
        hotelId: 37,
        insuranceId: 1,
        showId: 85,
        total: 107512.73,
      },
      {
        name: 'Salicylic Acid',
        quantPeople: 3,
        ticketId: 8,
        hotelId: 33,
        insuranceId: 1,
        showId: 30,
        total: 102932.71,
      },
      {
        name: 'Cephalexin',
        quantPeople: 4,
        ticketId: 60,
        hotelId: 69,
        insuranceId: 2,
        showId: 41,
        total: 60540.08,
      },
      {
        name: 'cyclobenzaprine hydrochloride',
        quantPeople: 4,
        ticketId: 34,
        hotelId: 55,
        insuranceId: 2,
        showId: 61,
        total: 133628.67,
      },
      {
        name: 'Phytolacca decandra, Ehrlichia, Mycoplasma pneumoniae, Borrelia burgdorferi, Babesia microti, Coxiella burnetii, Chelidonium majus, Herpes zoster,',
        quantPeople: 3,
        ticketId: 57,
        hotelId: 50,
        insuranceId: 1,
        showId: 9,
        total: 106478.17,
      },
      {
        name: 'Homosalate, Octinoxate, Octisalate, and Oxybenzone',
        quantPeople: 2,
        ticketId: 26,
        hotelId: 35,
        insuranceId: 2,
        showId: 26,
        total: 60913.25,
      },
      {
        name: 'NERIUM OLEANDER LEAF',
        quantPeople: 2,
        ticketId: 41,
        hotelId: 53,
        insuranceId: 1,
        showId: 4,
        total: 136020.83,
      },
      {
        name: 'HUMAN IMMUNEGLOBULIN G and HYALURONIDASE (HUMAN RECOMBINANT)',
        quantPeople: 2,
        ticketId: 45,
        hotelId: 71,
        insuranceId: 3,
        showId: 62,
        total: 107066.78,
      },
      {
        name: 'Broncofin',
        quantPeople: 3,
        ticketId: 31,
        hotelId: 90,
        insuranceId: 3,
        showId: 70,
        total: 168316.4,
      },
      {
        name: 'Zinc Oxide and Titanium Dioxide',
        quantPeople: 4,
        ticketId: 91,
        hotelId: 94,
        insuranceId: 1,
        showId: 65,
        total: 80312.28,
      },
      {
        name: 'Glimepiride',
        quantPeople: 2,
        ticketId: 36,
        hotelId: 72,
        insuranceId: 1,
        showId: 12,
        total: 120418.18,
      },
      {
        name: 'Cabbage',
        quantPeople: 2,
        ticketId: 16,
        hotelId: 36,
        insuranceId: 1,
        showId: 47,
        total: 99198.8,
      },
      {
        name: 'Homosalate, Octinoxate, Octisalate, and Oxybenzone',
        quantPeople: 2,
        ticketId: 26,
        hotelId: 85,
        insuranceId: 2,
        showId: 92,
        total: 139895.38,
      },
      {
        name: 'Fosinopril Sodium and Hydrochlorothiazide',
        quantPeople: 3,
        ticketId: 89,
        hotelId: 60,
        insuranceId: 3,
        showId: 88,
        total: 127095.42,
      },
      {
        name: 'Rivaroxaban',
        quantPeople: 2,
        ticketId: 4,
        hotelId: 13,
        insuranceId: 1,
        showId: 57,
        total: 159048.66,
      },
      {
        name: 'Terazosin Hydrochloride',
        quantPeople: 2,
        ticketId: 8,
        hotelId: 12,
        insuranceId: 2,
        showId: 71,
        total: 165621.42,
      },
      {
        name: 'triamcinolone acetonide',
        quantPeople: 3,
        ticketId: 67,
        hotelId: 46,
        insuranceId: 1,
        showId: 11,
        total: 184433.4,
      },
      {
        name: 'metoprolol tartrate',
        quantPeople: 6,
        ticketId: 67,
        hotelId: 74,
        insuranceId: 1,
        showId: 95,
        total: 127568.43,
      },
      {
        name: 'Thuja Occidentalis Leaf, Silicon Dioxide, and Sodium Chloride',
        quantPeople: 3,
        ticketId: 62,
        hotelId: 67,
        insuranceId: 3,
        showId: 49,
        total: 195574.42,
      },
      {
        name: 'CARBON DIOXIDE',
        quantPeople: 2,
        ticketId: 65,
        hotelId: 32,
        insuranceId: 3,
        showId: 15,
        total: 166069.79,
      },
      {
        name: 'cyclobenzaprine hydrochloride',
        quantPeople: 4,
        ticketId: 45,
        hotelId: 95,
        insuranceId: 1,
        showId: 100,
        total: 160805.16,
      },
      {
        name: 'White Poplar',
        quantPeople: 1,
        ticketId: 46,
        hotelId: 54,
        insuranceId: 3,
        showId: 64,
        total: 83735.31,
      },
      {
        name: 'Minoxidil',
        quantPeople: 1,
        ticketId: 11,
        hotelId: 53,
        insuranceId: 3,
        showId: 57,
        total: 126982.66,
      },
      {
        name: 'Capsicum Annuum, Foeniculum Vulgare, Symphytum Officinale, Argentum Nitricum, Arsenicum Album, Magnesia Phosphorica',
        quantPeople: 6,
        ticketId: 97,
        hotelId: 70,
        insuranceId: 1,
        showId: 38,
        total: 118736.24,
      },
      {
        name: 'octinoxate, octisalate, and zinc oxide',
        quantPeople: 4,
        ticketId: 86,
        hotelId: 71,
        insuranceId: 3,
        showId: 23,
        total: 124304.49,
      },
      {
        name: 'Simvastatin',
        quantPeople: 1,
        ticketId: 62,
        hotelId: 3,
        insuranceId: 1,
        showId: 94,
        total: 81223.51,
      },
      {
        name: 'Ibuprofen',
        quantPeople: 2,
        ticketId: 57,
        hotelId: 23,
        insuranceId: 2,
        showId: 50,
        total: 156216.62,
      },
      {
        name: 'Green Ash',
        quantPeople: 5,
        ticketId: 20,
        hotelId: 55,
        insuranceId: 1,
        showId: 75,
        total: 183532.72,
      },
      {
        name: 'losartan potassium',
        quantPeople: 4,
        ticketId: 42,
        hotelId: 24,
        insuranceId: 2,
        showId: 82,
        total: 140071.76,
      },
      {
        name: 'Ensulizole Octinoxate',
        quantPeople: 6,
        ticketId: 5,
        hotelId: 5,
        insuranceId: 3,
        showId: 73,
        total: 91708.49,
      },
      {
        name: 'Magnesium Sulfate',
        quantPeople: 1,
        ticketId: 15,
        hotelId: 17,
        insuranceId: 2,
        showId: 44,
        total: 132791.28,
      },
      {
        name: 'Black Olive',
        quantPeople: 3,
        ticketId: 84,
        hotelId: 39,
        insuranceId: 1,
        showId: 41,
        total: 140968.31,
      },
      {
        name: 'famciclovir',
        quantPeople: 2,
        ticketId: 33,
        hotelId: 10,
        insuranceId: 1,
        showId: 71,
        total: 90370.18,
      },
      {
        name: 'Benzalkonium chloride',
        quantPeople: 5,
        ticketId: 57,
        hotelId: 29,
        insuranceId: 1,
        showId: 99,
        total: 52535.47,
      },
      {
        name: 'acetaminophen',
        quantPeople: 4,
        ticketId: 29,
        hotelId: 49,
        insuranceId: 3,
        showId: 17,
        total: 136911.67,
      },
      {
        name: 'Metoprolol tartrate',
        quantPeople: 1,
        ticketId: 62,
        hotelId: 97,
        insuranceId: 1,
        showId: 71,
        total: 72603.25,
      },
      {
        name: 'Aluminum Zirconium Trichlorohydrex Gly',
        quantPeople: 5,
        ticketId: 66,
        hotelId: 45,
        insuranceId: 1,
        showId: 51,
        total: 150001.71,
      },
      {
        name: 'Diphenhydramine HCl',
        quantPeople: 1,
        ticketId: 41,
        hotelId: 24,
        insuranceId: 3,
        showId: 37,
        total: 73189.32,
      },
      {
        name: 'Naloxone Hydrochloride',
        quantPeople: 2,
        ticketId: 36,
        hotelId: 66,
        insuranceId: 2,
        showId: 27,
        total: 98369.46,
      },
      {
        name: 'SODIUM FLUORIDE',
        quantPeople: 6,
        ticketId: 9,
        hotelId: 52,
        insuranceId: 2,
        showId: 60,
        total: 87300.49,
      },
      {
        name: 'oxandrolone',
        quantPeople: 2,
        ticketId: 43,
        hotelId: 2,
        insuranceId: 1,
        showId: 72,
        total: 118521.17,
      },
      {
        name: 'Avobenzone, octocrylene, and oxybenzone',
        quantPeople: 4,
        ticketId: 74,
        hotelId: 90,
        insuranceId: 3,
        showId: 47,
        total: 170246.98,
      },
      {
        name: 'Methotrexate Sodium',
        quantPeople: 1,
        ticketId: 93,
        hotelId: 4,
        insuranceId: 2,
        showId: 87,
        total: 119703.92,
      },
      {
        name: 'Phenobarbital',
        quantPeople: 1,
        ticketId: 77,
        hotelId: 61,
        insuranceId: 1,
        showId: 72,
        total: 64951.25,
      },
      {
        name: 'Escitalopram',
        quantPeople: 1,
        ticketId: 7,
        hotelId: 88,
        insuranceId: 1,
        showId: 41,
        total: 143811.46,
      },
      {
        name: 'Venlafaxine Hydrochloride',
        quantPeople: 4,
        ticketId: 55,
        hotelId: 17,
        insuranceId: 2,
        showId: 76,
        total: 62911.07,
      },
      {
        name: 'Omeprazole',
        quantPeople: 4,
        ticketId: 83,
        hotelId: 94,
        insuranceId: 3,
        showId: 8,
        total: 90830.32,
      },
      {
        name: 'Losartan Potassium',
        quantPeople: 1,
        ticketId: 31,
        hotelId: 26,
        insuranceId: 3,
        showId: 7,
        total: 160736.88,
      },
      {
        name: 'soybean',
        quantPeople: 6,
        ticketId: 70,
        hotelId: 30,
        insuranceId: 1,
        showId: 77,
        total: 144264.95,
      },
      {
        name: 'HYPERICUM PERFORATUM',
        quantPeople: 5,
        ticketId: 71,
        hotelId: 18,
        insuranceId: 2,
        showId: 15,
        total: 186619.78,
      },
      {
        name: 'LEVOFLOXACIN',
        quantPeople: 6,
        ticketId: 25,
        hotelId: 79,
        insuranceId: 3,
        showId: 64,
        total: 83605.2,
      },
      {
        name: 'LISINOPRIL AND HYDROCHLOROTHIAZIDE',
        quantPeople: 5,
        ticketId: 49,
        hotelId: 31,
        insuranceId: 1,
        showId: 27,
        total: 117097.46,
      },
      {
        name: 'hydrocortisone',
        quantPeople: 2,
        ticketId: 88,
        hotelId: 39,
        insuranceId: 1,
        showId: 80,
        total: 150288.13,
      },
      {
        name: 'Avobenzone, Octinoxate, and Octisalate',
        quantPeople: 4,
        ticketId: 85,
        hotelId: 57,
        insuranceId: 3,
        showId: 41,
        total: 67621.55,
      },
      {
        name: 'NITROGEN',
        quantPeople: 3,
        ticketId: 74,
        hotelId: 81,
        insuranceId: 2,
        showId: 12,
        total: 101963.34,
      },
      {
        name: 'OCTINOXATE',
        quantPeople: 3,
        ticketId: 31,
        hotelId: 7,
        insuranceId: 2,
        showId: 35,
        total: 184828.03,
      },
      {
        name: 'Guaifenesin',
        quantPeople: 3,
        ticketId: 25,
        hotelId: 44,
        insuranceId: 1,
        showId: 19,
        total: 112543.7,
      },
      {
        name: 'TRIAMCINOLONE ACETONIDE',
        quantPeople: 4,
        ticketId: 62,
        hotelId: 31,
        insuranceId: 1,
        showId: 89,
        total: 128146.52,
      },
      {
        name: 'Etodolac',
        quantPeople: 4,
        ticketId: 2,
        hotelId: 77,
        insuranceId: 2,
        showId: 35,
        total: 188616.85,
      },
      {
        name: 'oxycodone hydrochloride and acetaminophen',
        quantPeople: 6,
        ticketId: 79,
        hotelId: 45,
        insuranceId: 3,
        showId: 8,
        total: 101378.07,
      },
      {
        name: 'Veratrum Chamomilla',
        quantPeople: 2,
        ticketId: 75,
        hotelId: 30,
        insuranceId: 2,
        showId: 17,
        total: 145984.0,
      },
      {
        name: 'Propranolol Hydrochloride',
        quantPeople: 1,
        ticketId: 37,
        hotelId: 51,
        insuranceId: 1,
        showId: 16,
        total: 143750.71,
      },
      {
        name: 'Atomoxetine hydrochloride',
        quantPeople: 1,
        ticketId: 39,
        hotelId: 4,
        insuranceId: 2,
        showId: 87,
        total: 90498.9,
      },
      {
        name: 'Antihemophilic Factor, Human Recombinant',
        quantPeople: 4,
        ticketId: 25,
        hotelId: 32,
        insuranceId: 2,
        showId: 68,
        total: 140483.98,
      },
      {
        name: 'methylprednisolone sodium succinate',
        quantPeople: 3,
        ticketId: 44,
        hotelId: 91,
        insuranceId: 2,
        showId: 66,
        total: 93962.09,
      },
      {
        name: 'Alendronate Sodium',
        quantPeople: 5,
        ticketId: 17,
        hotelId: 54,
        insuranceId: 1,
        showId: 96,
        total: 52845.76,
      },
      {
        name: 'HYDROCODONE BITARTRATE AND ACETAMINOPHEN',
        quantPeople: 4,
        ticketId: 9,
        hotelId: 54,
        insuranceId: 1,
        showId: 34,
        total: 113632.6,
      },
      {
        name: 'Guaifenesin',
        quantPeople: 3,
        ticketId: 13,
        hotelId: 25,
        insuranceId: 1,
        showId: 15,
        total: 55361.24,
      },
      {
        name: 'Threefold Balance',
        quantPeople: 6,
        ticketId: 46,
        hotelId: 57,
        insuranceId: 3,
        showId: 93,
        total: 75850.38,
      },
      {
        name: 'Human Insulin',
        quantPeople: 3,
        ticketId: 15,
        hotelId: 100,
        insuranceId: 3,
        showId: 54,
        total: 178449.4,
      },
      {
        name: 'Meloxicam',
        quantPeople: 6,
        ticketId: 38,
        hotelId: 2,
        insuranceId: 2,
        showId: 88,
        total: 96820.32,
      },
      {
        name: 'Colloidal silver,',
        quantPeople: 4,
        ticketId: 34,
        hotelId: 90,
        insuranceId: 2,
        showId: 80,
        total: 163608.89,
      },
      {
        name: 'Butalbital and Acetaminophen',
        quantPeople: 3,
        ticketId: 68,
        hotelId: 62,
        insuranceId: 2,
        showId: 74,
        total: 115957.66,
      },
      {
        name: 'Polyethylene Glycol 3350',
        quantPeople: 4,
        ticketId: 84,
        hotelId: 11,
        insuranceId: 1,
        showId: 10,
        total: 119053.67,
      },
      {
        name: 'Alendronate Sodium',
        quantPeople: 2,
        ticketId: 77,
        hotelId: 49,
        insuranceId: 2,
        showId: 34,
        total: 78619.04,
      },
      {
        name: 'HYDROQUINONE',
        quantPeople: 4,
        ticketId: 56,
        hotelId: 15,
        insuranceId: 1,
        showId: 84,
        total: 127526.27,
      },
      {
        name: 'Oxygen',
        quantPeople: 2,
        ticketId: 12,
        hotelId: 38,
        insuranceId: 3,
        showId: 9,
        total: 170523.41,
      },
      {
        name: 'Chlorpheniramine Maleate and Codeine Phosphate',
        quantPeople: 2,
        ticketId: 39,
        hotelId: 26,
        insuranceId: 2,
        showId: 25,
        total: 116759.85,
      },
      {
        name: 'Bassia hyssopifolia',
        quantPeople: 1,
        ticketId: 82,
        hotelId: 52,
        insuranceId: 3,
        showId: 91,
        total: 116526.92,
      },
      {
        name: 'Salicylic Acid',
        quantPeople: 6,
        ticketId: 81,
        hotelId: 97,
        insuranceId: 2,
        showId: 63,
        total: 112970.6,
      },
      {
        name: 'Acetaminophen',
        quantPeople: 1,
        ticketId: 20,
        hotelId: 27,
        insuranceId: 2,
        showId: 100,
        total: 193019.06,
      },
      {
        name: 'Acetaminophen, Dextromethorphan HBr, Doxylamine Succinate',
        quantPeople: 5,
        ticketId: 36,
        hotelId: 57,
        insuranceId: 2,
        showId: 60,
        total: 99171.1,
      },
      {
        name: 'Leucovorin Calcium',
        quantPeople: 3,
        ticketId: 75,
        hotelId: 53,
        insuranceId: 2,
        showId: 28,
        total: 190740.29,
      },
      {
        name: 'STANNOUS FLUORIDE',
        quantPeople: 2,
        ticketId: 82,
        hotelId: 47,
        insuranceId: 1,
        showId: 74,
        total: 109645.29,
      },
      {
        name: 'Octinoxate and Titanium Dioxide',
        quantPeople: 1,
        ticketId: 39,
        hotelId: 89,
        insuranceId: 3,
        showId: 74,
        total: 144467.4,
      },
      {
        name: 'Clobetasol Propionate',
        quantPeople: 3,
        ticketId: 29,
        hotelId: 25,
        insuranceId: 1,
        showId: 84,
        total: 162314.01,
      },
      {
        name: 'Caffeine',
        quantPeople: 4,
        ticketId: 13,
        hotelId: 13,
        insuranceId: 3,
        showId: 70,
        total: 91740.56,
      },
      {
        name: 'Benzocain',
        quantPeople: 2,
        ticketId: 2,
        hotelId: 5,
        insuranceId: 1,
        showId: 61,
        total: 155643.81,
      },
      {
        name: 'METOCLOPRAMIDE HYDROCHLORIDE',
        quantPeople: 3,
        ticketId: 87,
        hotelId: 21,
        insuranceId: 3,
        showId: 64,
        total: 194442.84,
      },
      {
        name: 'albuterol sulfate',
        quantPeople: 2,
        ticketId: 98,
        hotelId: 15,
        insuranceId: 3,
        showId: 72,
        total: 138430.31,
      },
      {
        name: 'ALLANTOIN',
        quantPeople: 1,
        ticketId: 74,
        hotelId: 88,
        insuranceId: 3,
        showId: 73,
        total: 66618.0,
      },
      {
        name: 'Venlafaxine Hydrochloride',
        quantPeople: 5,
        ticketId: 79,
        hotelId: 6,
        insuranceId: 1,
        showId: 85,
        total: 182505.43,
      },
      {
        name: 'strophanthus hispidus',
        quantPeople: 6,
        ticketId: 87,
        hotelId: 33,
        insuranceId: 3,
        showId: 27,
        total: 68743.34,
      },
      {
        name: 'CARISOPRODOL',
        quantPeople: 4,
        ticketId: 68,
        hotelId: 78,
        insuranceId: 2,
        showId: 29,
        total: 153689.74,
      },
      {
        name: 'Triclosan',
        quantPeople: 2,
        ticketId: 14,
        hotelId: 23,
        insuranceId: 2,
        showId: 17,
        total: 189059.63,
      },
      {
        name: 'Arnica montana HPUS',
        quantPeople: 5,
        ticketId: 45,
        hotelId: 67,
        insuranceId: 2,
        showId: 57,
        total: 59223.82,
      },
      {
        name: 'HISTAMINE DIHYDROCHLORIDE',
        quantPeople: 1,
        ticketId: 37,
        hotelId: 20,
        insuranceId: 2,
        showId: 80,
        total: 151234.34,
      },
      {
        name: 'Acetaminophen and Codeine Phosphate',
        quantPeople: 3,
        ticketId: 16,
        hotelId: 40,
        insuranceId: 1,
        showId: 68,
        total: 138542.81,
      },
      {
        name: 'ASCORBIC ACID',
        quantPeople: 6,
        ticketId: 26,
        hotelId: 37,
        insuranceId: 1,
        showId: 82,
        total: 70051.58,
      },
    ]);
  },

  async down(queryInterface, Sequelize) {
    return queryInterface.bulkDelete('Packages', null, {});
  },
};
