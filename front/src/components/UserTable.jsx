...
import ModalConfirmar from "./ModalesConfirmacionBorrar/ModalConfirmarBorrarUsuario";

export const UserTables = (props) => {
    ...
    return (
        ...
        {eliminar && <ModalConfirmar id={id} open={eliminar} setOpen={setEliminar} message='usuario'/>}
        ...
        <Link to={"/admin"}>
            ...
            <h2 className="text-2xl font-semibold leading-tight inline text-center ml-2">Usuarios</h2>
        </div>
		...
		<table className="w-full p-6 text-xs text-left text-white whitespace-nowrap">
            ...
            {users.map((user) => (
                    <tr key={user.id}>
                        ...
                        <td className="px-3 py-2"><button onClick={()=>{setEliminar(true);setId(user.id)}}>
                            <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
